...
class Solution:
    def minGroups(self, intervals: List[List[int]]) -> int:
        def canPlace(group, item):
            for t in group:
                if not (item[1] <= t[0] or item[0] >= t[1]):
                    return False
            return True
